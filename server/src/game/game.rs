...
use common::{
    data::cards::{Pack, Prompt, Response},
    protocol::serverbound::ServerBoundPacket,
};
...
impl Game {
    ...
    fn initialize_prompts(&mut self) {
        for (index, pack) in self.packs.iter().enumerate() {
            self.available_prompts
                .extend((0 .. pack.prompts.len()).map(|j| (index, j)));
        }
    }
    ...
    fn initialize_responses(&mut self) {
        for (index, pack) in self.packs.iter().enumerate() {
            self.available_responses
                .extend((0 .. pack.responses.len()).map(|j| (index, j)));
        }
    }
    ...
    fn select_response(&mut self) -> Response {
        ...
        let (pack, choice) =
            self.available_responses[thread_rng().gen_range(0 .. self.available_responses.len())];
