...
pub struct Pack {
    ...
    pub official: bool,
    #[serde(rename = "white")]
    #[serde(deserialize_with = "deserialize_response", serialize_with = "serialize_response")]
...
fn deserialize_response<'de, D>(d: D) -> Result<Vec<Response>, D::Error> where D: Deserializer<'de> {
    let v = d.deserialize_seq(ResponseVisitor)?;
    Ok(v.iter().map(|r| r.text.to_owned()).collect())
}

fn serialize_response<S>(responses: &Vec<Response>, s: S) -> Result<S::Ok, S::Error> where S: Serializer {
